<template>
  <div id="wallpaper" class="full-div">
    ...
    <div ref="window-taskbar" class="navbar-desktop">
      ...
      <div class="logo" v-on:click="redirect('Compétences')"  >
        <img alt="Mes compétences" title="Mes competences" src="@/assets/icons/skills2.png" />
      </div>
      <div class="logo" v-on:click="redirect('Diplômes')"  >
        <img alt="Mes diplômes" title="Mes diplômes" src="@/assets/icons/degree2.png" />
      </div>
      <div class="logo" v-on:click="redirect('Cv')"  >
      ...
      <div class="logo" v-on:click="redirect('Contact')"  >
        <img alt="contact" title="contact" src="@/assets/icons/contact2.png" />
      </div>
      <div class="logo" v-on:click="redirect('Github')"  >
        <img alt="github" title="github" src="@/assets/icons/github2.png" />
      </div>
      <div class="logo" v-on:click="redirect('Linkedin')">
...
export default class Kernel extends Vue {
  private ShortCutData: ShortCutIcon[] = [
    ...
    { name: 'Expériences', link: '/images/icons/work.png', color: '#F8F8F8' },
    { name: 'Compétences', link: '/images/icons/skills.png', color: '#84F0DB' },
    { name: 'Diplômes', link: '/images/icons/degree2.png', color: 'transparent' },
  ...
  get windowsReduced() {
    const newarray: PortfolioWindowReduced[] = [];
  ...
  public mounted() {
    ...
    this.$root.$on('Window::Props::Open', (item: SlotItem) => {
      ...
      newWindow.instance.$on('Window::Close', (item: PortfolioWindowPropsReduced) => {
        const fakeWindow: PortfolioWindowProps | undefined = this.windowsProps.get(item.id);
      ...
      newWindow.instance.$on('Window::Focus::Get', (item: PortfolioWindowPropsReduced) => {
        this.FocusManagementWindow(item.id);
      });
      this.windows.set(newWindow.id.toString(), newWindow);
      const element: Element = (this.$refs.windowcontainer as Element);
      if (!_.isNil(element)) {
        element.appendChild(newWindow.instance.$el);
      }
    });

    // WINDOWS NORMAL SETTINGS
    this.$root.$on('Window::Open', (item: SlotItem) => {
      const newWindow: PortfolioWindow = createWindow(item.title, item.component);
      ...
      newWindow.instance.$on('Window::Close', (item: PortfolioWindowReduced) => {
        const fakeWindow: PortfolioWindow | undefined = this.windows.get(item.id);
        if (!fakeWindow || _.isNil(fakeWindow)) {
          return;
        }
        // fakeWindow.instance.$destroy();
  ...
  private boot() {
    if (this.bootMenu === true) { this.bootMenu = false; } else { this.bootMenu = true; }
  }

  private openReducedWindow(id: string) {
    console.log(`Ouverture d'une fenetre reduite ${id}`);
    const fakeWindow: PortfolioWindow | undefined = this.windows.get(id);
    if (!fakeWindow || _.isNil(fakeWindow)) {
